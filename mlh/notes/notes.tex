...
\noindent Github Actions includes default environment variables for each workflow run. If you need custom ones, you set these in the YAML. In this example, we create variables \texttt{MYSQL_HOST} and \texttt{MYSQL_USER} which is connected to \texttt{app.py}
...
\noident You can also run scripts and shell commands, which are then executed by assigned runner. This next example has the command \texttt{npm install -g bats} run on a runner(1) and then how to run a script as an action you can store the script in your repo and supply the path and shell type(2):
