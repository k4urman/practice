...
\noindent \texttt{for} loops can also be written as \texttt{while} and \texttt{do-while} loops as so with the previous example:
...
\noindent Functions are blocks of code that perform a specific task and can be called upon multiple times. They are useful since they can be reused over and over again. We've already seen one called \texttt{main} that return an \texttt{int} value. Let's create another and call it in \texttt{main}.

\begin{lstlisting}[language=c, mathescape=true]
int adding(int n, int m) {
    return n + m;
}

int main() {
    int x, y;
    cout << "Please insert 2 numbers to add" << endl;
    cin >> x >> y;
    adding(x,y);
    return 0
}
